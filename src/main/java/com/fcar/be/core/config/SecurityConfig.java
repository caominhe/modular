...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception {
        httpSecurity
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers(
                                "/auth/**",           // Cho phép API login hệ thống và login Google (/auth/google/exchange)
                                "/users/register",    // Cho phép đăng ký chay (nếu bạn vẫn giữ API này)
                                "/swagger-ui/**",     // Cho phép xem tài liệu API
                                "/v3/api-docs/**"
                        ).permitAll()
                        .anyRequest().authenticated() // Mọi API khác (Bao gồm cả /users/onboard) đều bắt buộc phải có JWT Token
                )
    ...
    public CorsFilter corsFilter() {
        ...
        // Cho phép gửi kèm cookie hoặc thông tin xác thực (Bắt buộc phải có để Frontend gửi Header Authorization Bearer)
